/*
TABELA HASH EXTENSÃ�VEL

Os nomes dos mÃ©todos foram mantidos em inglÃªs
apenas para manter a coerÃªncia com o resto da
disciplina:
...
public class HashExtensivel<T extends RegistroHashExtensivel<T>> {
  ...
  public class Cesto {
    ...
    short quantidadeMaxima; // quantidade mÃ¡xima de elementos que o cesto pode conter
    ArrayList<T> elementos; // sequÃªncia de elementos armazenados
    ...
    public Cesto(Constructor<T> ct, int qtdmax, int pl) throws Exception {
      construtor = ct;
      if (qtdmax > 32767)
        throw new Exception("Quantidade mÃ¡xima de 32.767 elementos");
      if (pl > 127)
        throw new Exception("Profundidade local mÃ¡xima de 127 bits");
    ...
    public boolean create(T elem) {
      if (full())
        return false;
      int i = quantidade - 1; // posiÃ§Ã£o do Ãºltimo elemento no cesto
  ...
  protected class Diretorio {
    ...
    // sÃ³ serÃ£o considerados valores absolutos da chave.
    protected int hash(int chave) {
      return Math.abs(chave) % (int) Math.pow(2, profundidadeGlobal);
    }

    // MÃ©todo auxiliar para atualizar endereÃ§o ao duplicar o diretÃ³rio
  ...
  public HashExtensivel(Constructor<T> ct, int n, String nd, String nc) throws Exception {
    ...
    // Se o diretÃ³rio ou os cestos estiverem vazios, cria um novo diretÃ³rio e lista
    // de cestos
    if (arqDiretorio.length() == 0 || arqCestos.length() == 0) {

      // Cria um novo diretÃ³rio, com profundidade de 0 bits (1 Ãºnico elemento)
      diretorio = new Diretorio();
      byte[] bd = diretorio.toByteArray();
      arqDiretorio.write(bd);

      // Cria um cesto vazio, jÃ¡ apontado pelo Ãºnico elemento do diretÃ³rio
  ...
  public boolean create(T elem) throws Exception {
    ...
    // Identifica a hash do diretÃ³rio,
    ...
    // Testa se a chave jÃ¡ nÃ£o existe no cesto
    if (c.read(elem.hashCode()) != null)
      throw new Exception("Elemento já existe");

    // Testa se o cesto jÃ¡ nÃ£o estÃ¡ cheio
    // Se nÃ£o estiver, create o par de chave e dado
    if (!c.full()) {
    ...
    // Atualiza os dados no diretÃ³rio
    ...
    // Atualiza o arquivo do diretÃ³rio
  ...
  public void print() {
    try {
      ...
      while (arqCestos.getFilePointer() != arqCestos.length()) {
        System.out.println("EndereÃ§o: " + arqCestos.getFilePointer());
        Cesto c = new Cesto(construtor, quantidadeDadosPorCesto);
        byte[] ba = new byte[c.size()];
        arqCestos.read(ba);
        c.fromByteArray(ba);
